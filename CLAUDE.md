...
- Explicitly not built yet: any ticket list/detail UI, ticket update (status/category/assignment), AI classification/summaries/replies, an actual provider integration (this endpoint's JSON shape is intentionally generic, not SendGrid's or Mailgun's native payload format), outbound email sending, and reply-threading (this endpoint always creates a new ticket — it never matches an inbound email to an existing one).
