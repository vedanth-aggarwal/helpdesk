...
- Client uses `better-auth/react`'s `createAuthClient` (`client/src/lib/auth-client.ts`) pointed at `VITE_SERVER_URL`. `authClient.useSession()` drives both `ProtectedRoute` and `PublicOnlyRoute` (`client/src/components/`), which gate the route tree in `App.tsx` — protected pages render inside `Layout`, which also owns sign-out.
