...
Current state: auth/login, user management, and a bare-bones inbound-email-to-ticket ingestion endpoint are built (see `implementation-plan.md` for the full phased plan). Ticket CRUD (the agent-facing list/detail UI and API), AI features, a real email provider integration, outbound sending, and reply-threading are not yet implemented — see the Tickets section below for exactly what does and doesn't exist yet.
...
- Mock `@/lib/api` with `vi.mock` rather than hitting a real server — component tests should be fast and isolated. Same idea for anything using `authClient` (`@/lib/auth-client`) — mock `useSession`/`signIn`/`signOut` rather than hitting Better Auth for real.
...
- Explicitly not built yet: ticket detail view, ticket update (status/category/assignment), filtering/pagination on the list, AI classification/summaries/replies, an actual email provider integration (the inbound webhook's JSON shape is intentionally generic, not SendGrid's or Mailgun's native payload format), outbound email sending, and reply-threading (the webhook always creates a new ticket — it never matches an inbound email to an existing one).
